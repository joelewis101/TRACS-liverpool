...
for (sample_file in sample_files) {
  ...
  cat("Receipt sheets: ", receipt_sheets, "\n")
  ...
  cat("\nqPCR result sheets: ", qpcr_sheets, "\n")
  if (any(grepl("#98 qPCR results", qpcr_sheets)) &
    ...
    cat("Dropping #98 qPCR results - it's a retest carried ovee to the next sheet.\n")
  ...
  if (length(qpcr_sheets > 1)) {
    ...
    cat("\n", nrow(prev_weeks_samples), "that look like they are qPCR reruns - removing to add back in at the end")
  ...
  cat("\nMALDI result sheets: ", maldi_sheets, "\n")
